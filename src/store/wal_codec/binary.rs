...
pub(in crate::store) enum BinaryWalRecordDecode {
    Entry {
        entry: WalEntry,
        #[allow(dead_code)]
        commit_epoch: u64,
...
pub(in crate::store) fn encode_binary_wal_record(entry: &WalEntry, commit_epoch: u64) -> Vec<u8> {
...
pub(in crate::store) fn decode_binary_wal_record(bytes: &[u8]) -> Result<BinaryWalRecordDecode> {
...
mod tests {
    use super::*;
    use crate::store::wal_codec::wire;
    ...
    fn assert_op_round_trips(op: WalOp) {
        ...
        // WalOp does not implement PartialEq; the stable text encoding is
        // the canonical identity for comparison.
