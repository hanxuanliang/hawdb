...
mod tests {
    ...
    fn knowledge_retrieval_applies_rank_window_to_hybrid_search() {
        ...
        .unwrap();
        db.query("CREATE (:Memory {id: 'top_text', title: 'Graph seed', content: 'graph graph retrieval'})")
            .unwrap();
        db.query(
            "CREATE (:Memory {id: 'second_text', title: 'Fallback', content: 'graph context'})",
        )
        .unwrap();
    ...
    fn retrieves_knowledge_neighbors_without_search_projection() {
        let mut db = Database::new();
        db.query("CREATE (:Memory {id: 'root', title: 'Root'})-[:LINKS]->(:Entity {id: 'mid', name: 'Mid'})")
            .unwrap();
        let leaf = db
            .store
            .create_node(
    ...
    fn schema_ddl_creates_catalog_tokens_idempotently() {
        ...
        assert!(properties.iter().any(|property| {
            property.name == "weight"
                && property.value_type == PropertyType::Int
                && property.nullable
        }));
    ...
    fn full_text_index_ddl_enables_text_seek_plans() {
        ...
        let output = db
            .query(
                "MATCH (m:Memory) WHERE m.title CONTAINS 'Graph' RETURN m.id AS id ORDER BY id ASC",
            )
            .unwrap();
        assert_eq!(output.rows.len(), 2);
            ...
            assert!(db
                ...
                .any(|constraint| {
                    constraint.property == "id"
                        && constraint.kind == ConstraintKind::NodePropertyExists
                }));
        }
        std::fs::remove_dir_all(path).unwrap();
    }
    ...
    fn projects_graph_for_page_rank() {
        ...
        db.query("MERGE (:Entity {id: 2, name: 'Mid'})-[:LINKS]->(:Entity {id: 3, name: 'Leaf'})")
            .unwrap();
        db.query(
            "MERGE (:Entity {id: 3, name: 'Leaf'})-[:MENTIONS]->(:Entity {id: 4, name: 'Other'})",
        )
        .unwrap();
    ...
    fn projected_graph_definition_replays_from_wal() {
        let path = unique_test_dir("projected_graph_wal");
        {
            ...
            .unwrap();
            db.query(
                "MERGE (:Entity {id: 2, name: 'Mid'})-[:LINKS]->(:Entity {id: 3, name: 'Leaf'})",
            )
            .unwrap();
    ...
    fn exposes_property_index_descriptors_and_statistics() {
        ...
        db.query("CREATE (:Memory {id: 3, kind: 'decision'})-[:MENTIONS]->(:Entity {id: 10, name: 'Rust'})")
            .unwrap();
    ...
    fn bounded_multi_hop_statistics_drive_expand_estimates() {
        let mut db = Database::new();
        db.query(
            "CREATE (:Memory {id: 1, title: 'Root'})-[:LINKS]->(:Entity {id: 1000, name: 'Mid'})",
        )
        .unwrap();
        for id in 0..100 {
            db.query(&format!(
    ...
    fn expands_bounded_relationship_patterns() {
        ...
        let output = db
            .query(
                "MATCH (m:Memory)-[:LINKS*1..2]->(e:Entity) RETURN e.name AS name ORDER BY name ASC",
            )
            .unwrap();
        assert_eq!(output.rows.len(), 2);
        assert_eq!(
        ...
        assert!(explain.trace.decisions.iter().any(|decision| {
            decision
                .contains("estimate AdjacencyExpand for Memory-[:LINKS*1..2]->Entity: path_count=1")
    ...
    fn filters_with_literal_and_parameterized_in_predicates() {
        ...
        let output = db
            .query(
                "MATCH (m:Memory) WHERE m.id IN [1, 3] RETURN m.title AS title ORDER BY title ASC",
            )
            .unwrap();
        assert_eq!(output.rows.len(), 2);
        assert_eq!(
    ...
    fn filters_with_not_equal_predicates() {
        ...
        let output = db
            .query(
                "MATCH (m:Memory) WHERE m.kind <> 'task' RETURN m.title AS title ORDER BY title ASC",
            )
            .unwrap();
        assert_eq!(output.rows.len(), 2);
        assert_eq!(
    ...
    fn return_projection_functions_cover_nowledge_fallback_reads() {
        ...
        let output = db
            .query(
                "MATCH (m:Memory)-[r:MENTIONS]->(e:Entity) RETURN COALESCE(null, e.id) AS fallback",
            )
            .unwrap();
    ...
    fn predicate_projection_functions_cover_nowledge_fallback_filters() {
        ...
        .unwrap();
        db.query(
            "CREATE (:Memory {id: 3, title: 'Graph crystal', created_at: 20, is_crystal: true})",
        )
        .unwrap();
    ...
    fn optional_match_count_after_relationship_match_covers_legacy_tail_refs() {
        let mut db = Database::new();
        db.query(
            "CREATE (:Thread {id: 't1'})-[:CONTAINS]->(:Message {id: 'msg1', order_index: 1})",
        )
        .unwrap();
        db.query("CREATE (:Message {id: 'msg2', order_index: 2})")
            .unwrap();
        db.query(
            "MATCH (t:Thread {id: 't1'}), (m:Message {id: 'msg2'}) CREATE (t)-[:CONTAINS]->(m)",
        )
        .unwrap();
    ...
    fn optional_match_with_degree_projection_covers_top_entities() {
        ...
        db.query("CREATE (:Entity {id: 'e4', name: 'Isolated'})")
            .unwrap();
        db.query(
            "MATCH (a:Entity {id: 'e1'}), (b:Entity {id: 'e2'}) CREATE (a)-[:RELATES_TO]->(b)",
        )
        .unwrap();
        db.query(
            "MATCH (a:Entity {id: 'e2'}), (b:Entity {id: 'e3'}) CREATE (a)-[:RELATES_TO]->(b)",
        )
        .unwrap();
    ...
    fn node_detail_neighbor_counts_use_distinct_neighbors_and_edges() {
        ...
        db.query("CREATE (:Entity {id: 'n2', name: 'Two'})")
            .unwrap();
        db.query(
            "MATCH (a:Entity {id: 'n1'}), (b:Entity {id: 'n2'}) CREATE (a)-[:RELATES_TO]->(b)",
        )
        .unwrap();
    ...
    fn creates_and_filters_escaped_string_literals() {
        ...
        let output = db
            .query(
                r#"MATCH (m:Memory) WHERE m.title = 'It\'s graph\\ready' RETURN m.title AS title"#,
            )
            .unwrap();
        assert_eq!(output.rows.len(), 1);
        assert_eq!(
    ...
    fn returns_filters_orders_and_counts_relationship_properties() {
        ...
        let output = db
            .query("MATCH (m:Memory)-[r:MENTIONS]->(e:Entity) WHERE id(r) = 1 RETURN r.weight AS weight")
            .unwrap();
    ...
    fn merge_node_on_match_set_updates_only_when_matched() {
        let mut db = Database::new();
        db.query("CREATE (:Label {id: 'label-1', name: 'Important', canonical_name: null, updated_at: 1})")
            .unwrap();
    ...
    fn transaction_merge_on_create_set_deduplicates_pending_nodes_by_match_key() {
        let path = unique_test_dir("merge_on_create_transaction_batch");
        {
            ...
            .unwrap();
            tx.query("MERGE (m:SchemaMigrationLog {id: 'migration-1'}) ON CREATE SET m.note = 'duplicate'")
                .unwrap();
            let output = tx.commit().unwrap();
            assert_eq!(output.rows.len(), 2);
            assert_eq!(output.rows[0].get("created"), Some(&Value::Bool(true)));
    ...
    fn multi_label_match_reads_any_listed_label() {
        let mut db = Database::new();
        db.query("CREATE (:Memory {id: 1, title: 'Memory'})-[:MENTIONS]->(:Entity {id: 2, name: 'Entity'})")
            .unwrap();
    ...
    fn delete_rejects_nodes_with_relationships() {
        ...
        let output = db
            .query(
                "MATCH (m:Memory)-[:MENTIONS]->(e:Entity) WHERE m.id = 1 RETURN e.name AS entity",
            )
            .unwrap();
        assert_eq!(output.rows.len(), 1);
    }
