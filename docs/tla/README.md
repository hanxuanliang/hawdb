...
durable-before-visible publication. A concrete SQL statement may emit multiple
index changes. Entry/byte accounting, ordered base-plus-overlay merging, and
commit-time revalidation after concurrent base advancement remain concrete Rust
and `SkeinIndexPublication.tla` refinement obligations.
