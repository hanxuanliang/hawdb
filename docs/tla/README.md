...
workspace primitive separately from production-open activation. A staged row
delta overrides the immutable checkpoint, the caller must supply exactly the
authenticated access keys plus the correct present/missing shape, and the
workspace is admitted only below its hard entry bound. Replay changes only the
local workspace and the logical relation, exact row count publication occurs
atomically, and the workspace is discarded before a later record. Missing,
drifted, or oversized hydration enters a terminal rejected state without ever
...
inside the checksummed relational WAL envelope. Current writable recovery
recomputes the access set from materialized replay; the later sparse-writable
refinement will use the same authenticated set as its complete hydration domain.
...
| One schema-stable writable replay record can be evaluated in a materialized workspace containing exactly its authenticated present/missing access entries, with hard entry and conservative resident-byte admission, staged-delta-over-checkpoint lookup, differential access-set validation, exact detached-count advancement, bounded overflow retention, and workspace disposal; this primitive does not change the production writable-open selector | `RelationalRowDeltaBuilder::lookup_staged`, `RelationalState::{stage_sparse_transaction_for_authoritative_recovery_with_replay_access,hydrate_sparse_recovery_row_with_context}` | `unpublished_lookup_prefers_dirty_values_then_newest_immutable_run`, `sparse_authoritative_recovery_matches_materialized_predicate_replay`, `SkeinSparseWritableRelationalRecovery.tla`, `SkeinRelationalWalReplayAccess.tla` |
...
| The read-only production Content Store runner reopens one pinned serving view per case, obtains and releases one bounded foreground permit per measured read, rejects row/index epoch drift, and observes cold/warm demand reads without changing the modeled publication state | `run_production_content_store_storage_qualification` | `production_content_store_runner_binds_cold_warm_relational_residency`, `SkeinRuntimeAdmission.tla`, `SkeinRelationalRowSnapshotRead.tla`, `SkeinIndexRecovery.tla` |
| The writable production Content Store runner keeps the source read-only, commits disjoint frozen insert/update sequences on isolated 1/4/8/10-writer replicas, observes one epoch per durable transaction, reopens non-empty WAL into current row/index recovery views bound to the exact replay-source identity, then checkpoints and reopens a delta-free manifest-selected view with identical result digests | `RelationalRecoverySourceBuilder`, `RelationalIndexRecoveryBuilder::finish_with_recovery_source`, `run_production_content_store_mutation_qualification`, `ConcurrentDatabase::{storage_pressure_snapshot,storage_residency_report,storage_recovery_report}` | `recovery_source_binds_record_order_and_payload`, `production_mutation_runner_qualifies_isolated_writer_matrix`, `SkeinTransactionConcurrency.tla`, `SkeinWalGroupCommit.tla`, `SkeinStorageDurability.tla`, `SkeinRelationalRowSnapshotRead.tla`, `SkeinIndexRecovery.tla` |
