...
| Canonical segment checkpoints shadow-publish an order-preserving descriptor tree after the synchronized data artifact and bind its source epoch, count, and root integrity through the canonical manifest selected last. Normal open verifies only the bounded root and page length; explicit verification reads the complete page closure uncached and compares every decoded descriptor with the resident manifest oracle. The resident vector deliberately remains authoritative until a separate activation change. | `CanonicalSegmentDescriptor::{descriptor_tree_key,encode_descriptor_tree_value,decode_descriptor_tree_entry}`, `CanonicalSegmentWriter`, `CanonicalSegmentReader::verify_descriptor_shadow`, `DurableStore::{write_canonical_segments,load_published_canonical_segments}` | `canonical_descriptor_shadow_is_bound_lazy_and_exhaustively_verified`, `checkpoint_publishes_digest_bound_canonical_segments`, `backup_restore_publishes_one_verified_generation`, `SkeinGraphDescriptorPaging.tla` |
