...
| Canonical segment checkpoints publish an order-preserving descriptor tree after the synchronized data artifact and bind its source epoch, aggregate per-kind counts, and root integrity through the compact canonical manifest selected last. Normal open verifies only the bounded root and page length. Point reads lower-bound seek one descriptor; scans advance through fixed-size descriptor batches with independent page, byte, descriptor, and height admission, and iterators retain one decoded segment. Read reports separate descriptor and data cache/I/O. Deep scrub bypasses caches, hashes both artifacts, decodes every descriptor and record shape, and proves contiguous physical and logical closure. Physical failure sticky-poisons the reader while admission does not. The activation, non-poisoning admission, and pinned demand-reader transitions refine `SkeinGraphDescriptorPaging.tla`. | `CanonicalSegmentDescriptor::{descriptor_tree_key,encode_descriptor_tree_value,decode_descriptor_tree_entry}`, `CanonicalSegmentWriter`, `CanonicalSegmentReader::{get_node_with_report,get_relationship_with_report,scan_nodes_control,scan_relationships_control,deep_scrub}`, `DurableStore::{write_canonical_segments,load_published_canonical_segments}` | `canonical_descriptor_demand_reader_is_lazy_and_exhaustively_scrubbed`, `compact_manifest_residency_is_independent_of_segment_count`, `canonical_segment_admission_does_not_poison_the_reader`, `canonical_scan_streams_more_than_one_descriptor_batch`, `checkpoint_publishes_digest_bound_canonical_segments`, `backup_restore_publishes_one_verified_generation`, `SkeinGraphDescriptorPaging.tla` |
