...
| Exact overflow compaction scans one pinned metadata-only row closure without payload hydration, externally sorts references within memory/spill/run limits, rewrites every reachable envelope into a fresh physical generation, selects it only through the outer checkpoint manifest, and preserves current, previous, and reader-pinned roots until reclamation | `Database::compact_relational_overflow`, `GraphStore::collect_exact_relational_overflow_closure`, `RelationalOverflowReferenceSetBuilder`, `RelationalOverflowPublisher::persist_generation_exact_references`, `DurableStore::reclaim_old_generations` | `spilled_reference_set_is_repeatable_sorted_and_deduplicated`, `conflicting_metadata_for_one_digest_fails_closed`, `exact_overflow_compaction_rewrites_reachable_closure_without_hydration`, `SkeinOverflowExactCompaction.tla` |
