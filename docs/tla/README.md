...
| A graph descriptor page has a bounded, symmetric immutable encoding; parent references bind its exact physical generation, range, integrity digests, source epoch, artifact class, and key range. The accompanying model defines the future publish-last and bounded demand-read protocol, but no production manifest selects this format yet. | `ImmutableGraphDescriptorPage::{encode,encode_with_ref,decode,decode_bound}`, `GraphDescriptorPageRef` | `leaf_page_round_trips_with_exact_bound_reference`, `interior_page_round_trips_cross_generation_children`, `decoder_rejects_bit_flips_and_truncation`, `bound_decode_rejects_identity_digest_and_range_drift`, `large_descriptor_values_fit_without_changing_the_page_ceiling`, `SkeinGraphDescriptorPaging.tla` |
