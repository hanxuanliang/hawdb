...
512 MiB read. Advancing the release identity clears readiness while retaining
...
| Release admission requires current-identity desktop and explicit 512 MiB policy evidence plus distinct representative production and constrained 512 MiB reads; an identity advance makes every retained input stale until recollected | `run_production_content_store_memory_qualification`, `evaluate_production_release_qualification_bundle`, `release_bundle::memory` | `complete_raw_artifact_bundle_is_ready`, `memory_profile_matrix_rejects_different_resource_snapshots`, `production_read_cannot_substitute_the_512_mib_capability_run`, `configured_production_read_cannot_substitute_the_512_mib_capability_run`, `SkeinMemoryTierRelease.tla` |
| The read-only production Content Store runner reopens one pinned serving view per case, obtains and releases one bounded foreground permit per measured read, rejects row/index epoch drift, records sequential manifest/checkpoint-root/WAL/post-replay intervals inside one total-open interval, and observes cold/warm demand reads without changing the modeled publication state. The bounded CLI plan parser only constructs this typed runner and has no independent durable transition. | `GraphStore::{open_with_options,finish_open}`, `StorageOpenTimings`, `run_production_content_store_storage_qualification`, `skein-content-store-read-qualification` | `storage_recovery_report_tracks_wal_replay_boundary`, `production_content_store_runner_binds_cold_warm_relational_residency`, `SkeinStorageDurability.tla`, `SkeinRuntimeAdmission.tla`, `SkeinRelationalRowSnapshotRead.tla`, `SkeinIndexRecovery.tla` |
...
| Exact overflow compaction scans one pinned metadata-only row closure without payload hydration, externally sorts references within memory/spill/run limits, rewrites every reachable envelope into a fresh physical generation, selects it only through the outer checkpoint manifest, preserves current, previous, and reader-pinned roots until reclamation, and emits bound production resource plus physical-deletion evidence from a caller-owned disposable replica | `Database::compact_relational_overflow`, `GraphStore::collect_exact_relational_overflow_closure`, `RelationalOverflowReferenceSetBuilder`, `RelationalOverflowPublisher::persist_generation_exact_references`, `DurableStore::reclaim_old_generations`, `run_production_content_store_overflow_compaction_qualification` | `spilled_reference_set_is_repeatable_sorted_and_deduplicated`, `conflicting_metadata_for_one_digest_fails_closed`, `exact_overflow_compaction_rewrites_reachable_closure_without_hydration`, `qualification_proves_exact_rewrite_physical_reclaim_and_reopen`, `SkeinOverflowExactCompaction.tla` |
