...
| Property-projection equality, range, full-text, composite, and relationship blocks have an order-preserving symmetric descriptor codec and stream into the same bounded page-tree state machine. The data artifact is durable before immutable descriptor pages and the root is published last. This root remains an unselected shadow candidate, so a crash or root publication cannot alter the existing manifest-backed serving path; backup, repair, discard, and reclamation retain its generation closure. | `PersistentPropertyProjectionBlockDescriptor::{descriptor_tree_key,encode_descriptor_tree_value,decode_descriptor_tree_entry}`, `PersistentPropertyProjectionWriter::write_fallible_with_descriptor_tree`, `DurableStore::write_persistent_property_projection` | `descriptor_tree_entry_codec_is_order_preserving_and_symmetric`, `external_projection_round_trips_node_composite_and_relationship_candidates`, `SkeinGraphDescriptorPaging.tla` |
