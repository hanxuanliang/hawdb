...
| The read-only production Content Store runner reopens one pinned serving view per case, obtains and releases one bounded foreground permit per measured read, rejects row/index epoch drift, records sequential manifest/checkpoint-root/WAL/post-replay intervals inside one total-open interval, and observes cold/warm demand reads without changing the modeled publication state | `GraphStore::{open_with_options,finish_open}`, `StorageOpenTimings`, `run_production_content_store_storage_qualification` | `storage_recovery_report_tracks_wal_replay_boundary`, `production_content_store_runner_binds_cold_warm_relational_residency`, `SkeinStorageDurability.tla`, `SkeinRuntimeAdmission.tla`, `SkeinRelationalRowSnapshotRead.tla`, `SkeinIndexRecovery.tla` |
