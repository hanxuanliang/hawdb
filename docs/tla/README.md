...
| Property-spill blocks have an order-preserving fixed-width descriptor codec and stream into the same bounded tree builder. The spill data artifact becomes durable before immutable descriptor pages and the root, while the outer property manifest binds the source epoch, root identity, and legacy block-count oracle. Descriptor-root-only verification, backup, scrub, discard, and generation cleanup verify or retain the complete shadow closure without claiming that the legacy manifest is compact. The formal model now requires same-generation data durability before any descriptor-root publication; demand-read activation remains a later refinement. | `PropertySpillBlockDescriptor::{descriptor_tree_key,encode_descriptor_tree_value,decode_descriptor_tree_entry}`, `PropertySpillWriter::{create,finish}`, `PreparedPropertySpillArtifact::publish`, `DurableStore::{write_canonical_segments,load_published_property_spills}` | `descriptor_tree_entry_codec_is_order_preserving_and_symmetric`, `spill_blocks_round_trip_and_fail_closed_on_corruption`, `out_of_core_property_spills_round_trip_and_fail_closed`, `SkeinGraphDescriptorPaging.tla` |
