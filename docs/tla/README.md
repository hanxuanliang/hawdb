...
| Graph descriptor pages have a bounded, symmetric immutable encoding; parent references bind exact physical identity, range, integrity, source epoch, class, and key range. The canonical adjacency checkpoint streams descriptor references through bounded level runs, durably replaces the adjacency artifact, publishes an immutable page artifact, and publishes a bounded root last; root-only reopen reads no page payload. This remains an unselected shadow artifact: production serving continues through the legacy manifest until a later activation contract. | `ImmutableGraphDescriptorPage::{encode,encode_with_ref,decode,decode_bound}`, `GraphDescriptorTreeBuilder`, `PreparedGraphDescriptorTree::publish`, `GraphDescriptorTreeRootReader`, `CanonicalAdjacencyWriter::write_fallible_with_descriptor_tree`, `DurableStore::write_canonical_adjacency` | `leaf_page_round_trips_with_exact_bound_reference`, `streaming_tree_round_trips_with_bounded_residency`, `builder_rejects_non_converging_interior_fanout`, `publication_refuses_to_replace_an_existing_generation_artifact`, `page_artifact_is_durable_before_root_publication`, `corrupted_root_fails_closed_before_page_payload_reads`, `descriptor_tree_entry_codec_is_order_preserving_and_symmetric`, `writer_publishes_unselected_descriptor_root_after_adjacency_artifact`, `out_of_core_checkpoint_uses_generation_bound_dense_adjacency_and_fails_closed`, `SkeinGraphDescriptorPaging.tla` |
