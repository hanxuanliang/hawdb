...
| Compatibility front door | parameterized Cypher query-runtime shape for Nowledge calls | Partial: `NowledgeGraphAdapter` exposes parameterized query, explain, work-request inspection, grouped mutation transaction execution over `NowledgeGraphStatement`, and Knowledge Retrieval over a caller-owned `SearchIndex`. Query-shape-specific typed APIs remain on `Database` as migration compatibility surfaces for existing tests and callers, but they are no longer the adapter front door and should not expand unless an active migration caller cannot be expressed safely through Cypher. Public fixture and inventory constructors continue to define the production call-site contract, while coverage, inventory gate, shadow cutover, and migration gate reports remain the CI-facing replacement evidence boundary. |
