...
use skein::{Database, QueryOutput, QueryStreamOptions, Result, SkeinError, Value};
...
pub(super) fn qualify_multi_statement_transaction(
    ...
    let expected_rows = message_position.checked_add(1).ok_or_else(|| {
        SkeinError::Semantic("content-store transaction message count overflowed usize".to_string())
    })?;
